...
bool CheckUserinfoFlood(int clientNum) {
    ...
    if (test_server_time - cl->lastUserInfoChange < USERINFO_CHANGE_PERIOD) {
        ...
        if (cl->lastUserInfoCount > MAX_USERINFO_CHANGES) return false;
    } else {
...
class ClientUserinfoTest : public ::testing::Test {
protected:
    void SetUp() override {
        ResetUserinfo();
    }
...
TEST_F(ClientUserinfoTest, StoresUserinfo) {
    bool ok = SetUserinfo(0, "\\name\\Player\\rate\\25000");
    EXPECT_TRUE(ok);
    EXPECT_STREQ(test_clients[0].userinfo, "\\name\\Player\\rate\\25000");
}

TEST_F(ClientUserinfoTest, RejectsTooLong) {
...
TEST_F(ClientUserinfoTest, ExtractsName) {
    const char* name = Info_ValueForKey("\\name\\TestPlayer", "name");
    EXPECT_STREQ(name, "TestPlayer");
}

TEST_F(ClientUserinfoTest, ExtractsRate) {
...
TEST_F(ClientUserinfoTest, MissingKeyReturnsEmpty) {
...
TEST_F(ClientUserinfoTest, AllowsFirstChange) {
    bool ok = CheckUserinfoFlood(0);
    EXPECT_TRUE(ok);
}

TEST_F(ClientUserinfoTest, AllowsMultipleChanges) {
...
TEST_F(ClientUserinfoTest, BlocksExcessiveChanges) {
...
TEST_F(ClientUserinfoTest, ResetsAfterPeriod) {
...
TEST_F(ClientUserinfoTest, IndependentUserinfo) {
    SetUserinfo(0, "\\name\\Player1");
    SetUserinfo(1, "\\name\\Player2");
    UpdateDerivedInfo(0);
    UpdateDerivedInfo(1);
...
TEST_F(ClientUserinfoTest, IndependentFloodCheck) {
...
TEST_F(ClientUserinfoTest, EmptyUserinfo) {
    SetUserinfo(0, "");
    EXPECT_STREQ(test_clients[0].userinfo, "");
}

TEST_F(ClientUserinfoTest, EmptyValue) {
    const char* value = Info_ValueForKey("\\name\\", "name");
    EXPECT_STREQ(value, "");
}

TEST_F(ClientUserinfoTest, LongName) {
    char longname[100];
...
TEST_F(ClientUserinfoTest, ZeroRate) {
...
TEST_F(ClientUserinfoTest, ManyUpdates) {
    for (int i = 0; i < 100; i++) {
...
TEST_F(ClientUserinfoTest, AllClients) {
    for (int i = 0; i < 64; i++) {
        char info[MAX_INFO_STRING];
        snprintf(info, sizeof(info), "\\name\\Player%d", i);
        SetUserinfo(i, info);
